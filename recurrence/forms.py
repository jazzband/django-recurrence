...
class RecurrenceWidget(forms.Textarea):
    ...
    def get_media(self):
        ...
        return forms.Media(
            js=js,
            css={"all": (staticfiles_storage.url("recurrence/css/recurrence.css"),)},
        )
...
class RecurrenceField(forms.CharField):
    ...
    default_error_messages = {
        "invalid_frequency": _(u"Invalid frequency."),
        "max_rrules_exceeded": _(u"Max rules exceeded. The limit is %(limit)s"),
        "max_exrules_exceeded": _(
            u"Max exclusion rules exceeded. The limit is %(limit)s"
        ),
        "max_rdates_exceeded": _(u"Max dates exceeded. The limit is %(limit)s"),
